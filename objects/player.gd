...
func is_downhill() -> bool:
	...
	return normal.dot(get_forward()) > 0.0
...
func handle_slopes(delta:float,slope_assistance:float,slope_drag:float,flat_drag := 0.0) -> void:
	...
	if is_on_flat_ground():
		speed -= flat_drag * delta
	elif is_uphill():
		speed -= slope_drag * delta
	elif is_downhill():
		speed += slope_assistance * delta
...
func _on_homing_attack_state_physics_processing(delta: float) -> void:
